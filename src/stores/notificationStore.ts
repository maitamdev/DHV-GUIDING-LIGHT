...
export const useNotificationStore = create<NotificationStore>((set) => ({
  ...
  addNotification: (notification) => set(s => ({ notifications: [notification, ...s.notifications], unreadCount: s.unreadCount + (notification.isRead ? 0 : 1) })),
  removeNotification: (id) => set(s => {
    const n = s.notifications.find(n => n.id === id);
    const notifications = s.notifications.filter(n => n.id !== id);
    return { notifications, unreadCount: s.unreadCount - (n && !n.isRead ? 1 : 0) };
  }),
}));
